...
TEXT_COLLECTION: dict[str, dict[str, int]] = {
...
TEXT_QUERIES: dict[str, dict[str, int]] = {
...
def test_search_topk_only_touches_relevant_terms() -> None:
    # El accumulator solo deberia leer las posting lists de los terminos de la query, no todas. I/O seeks == # terminos validos en la query.
    with tempfile.TemporaryDirectory() as tmp:
        postings_path, vocab_path, meta_path = _build_full_text_index(tmp)
        with InvertedIndex(postings_path, vocab_path, meta_path) as idx:
